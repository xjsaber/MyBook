...
为了解决存储器大容量、低价格、高速度三者之间的矛盾关系，常采用多级存储器结构，常采用多级存储器结构。

寄存器（寄存器是有限存储容量的高速存储部件，它们可用来暂存指令、数据等）通常都是制作在CPU芯片内。寄存器中的数直接在CPU内部参与运算，CPU内可以有十几或者几十个寄存器。

Cache-主存-辅存各层次中的内容都可以在下层次中找到，即Cache的内容可以在主存和辅存中找到；主存的内容可以在辅存中找到。

* 缓存-主存层次主要解决CPU和主存速度不匹配的问题。**主存和缓存之间的数据交换是硬件自动完成的，对程序员是透明的**。
...
2. 虚拟存储器必须建立在主存-辅存结构基础上，并不是虚拟存储器最多只允许使用主存空间；虚拟存储器每次访问时，必须进行虚实地址变换，而非虚拟存储器则不必。
...
加/减/乘/除或其他操作（由操作码实现）。确定操作后就要知道对谁进行操作（由地址码实现）。一个指令必须由操作码或地址码两部分组成。
