...
使用synchronized关键字来控制一个方法的并发访问。如果一个对象已用synchronized关键字声明，那么只有一个执行线程被允许访问它。
...
缓冲区是一个共享数据结构，必须使用同步机制控制对它的访问。

Java在Object类中提供了wait()、notify()和notifyAll()方法。线程可以在同步代码块中调用wait()方法。如果在同步代码块之外调用wait()方法，JVM将抛出ILLegalMonitorStateException异常。当一个线程调用wait()方法时，JVM将这个线程置入休眠，并且释放控制这个同步代码块的对象，同时允许其他线程执行这个对象控制的其他代码块。为了唤醒这个线程，必须在这个对象控制的某个同步代码块中调用notify()或者notifyAll()方法。

**工作原理**

数据存储EventStorage类的set()和get()方法。
