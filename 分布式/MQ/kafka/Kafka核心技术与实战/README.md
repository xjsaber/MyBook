...
kafka有主题（Topic）的概念，承载着真实数据的容器，而主题之下还分为若干个分区，Kafka 的消息组织方式实际上是三级结构：主题 - 分区 - 消息。主题下的每条消息只会保存在某一个分区中，而不会在多个分区中被保存多份。
...
如果要自定义分区策略，你需要显式地配置生产者端的参数partitioner.class。在编写生产者程序时，你可以编写一个具体的类实现org.apache.kafka.clients.producer.Partitioner接口。这个接口也很简单，只定义了两个方法：partition()和close()，自定义实现通常`partition`：
...
也称 Round-robin 策略，即顺序分配。默认采用这种方式。
...
也称 Key-ordering 策略。Kafka 允许为每条消息定义消息键，简称为 Key。这个 Key 的作用非常大，它可以是一个有着明确业务含义的字符串，比如客户代码、部门编号或是业务 ID 等；也可以用来表征消息元数据。在一些场景中，工程师们都是直接将消息创建时间封装进 Key 里面的。一旦消息被定义了 Key，那么你就可以保证同一个 Key 的所有消息都进入到相同的分区里面，由于每个分区下的消息处理都是有顺序的，故这个策略被称为按消息键保序策略。
...
*当时碰到的一个问题就是如何实现消息的顺序问题。这家企业发送的 Kafka 的消息是有因果关系的，故处理因果关系也必须要保证有序性*

1. 原先的做法是将分区严格限制在一个，这样的话所有的消息都会通过这个分区，因为Kafka的分区保证有序性，保证了逻辑处理的有序性
2. 观察和分析因果关系的消息具有一定的特定，比如在消息体中都封装了固定的标志位，针对标志位设立专门的分区策略（同一标志位的所有消息都发送到同一分区），既可以保证分区内的消息顺序，也可以享受到多分区带来的性能红利。

### 其他分区策略 ###

*假设极客时间计划为每个新注册用户提供一份注册礼品，比如南方的用户注册极客时间可以免费得到一碗“甜豆腐脑”，而北方的新注册用户可以得到一碗“咸豆腐脑”。*根据不同的分区策略，
