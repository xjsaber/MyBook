...
任何 JavaScript 代码片段在执行前都要进行编译（通常就在执行前）。因此，JavaScript 编译器首先会对 var a = 2; 这段程序进行编译，然后做好执行它的准备，并且
通常马上就会执行它。
...
### 1.2.5 小测验 ###
	...
	var c= foo(2);
foo(RHS)，参数a（LHS），b=a对a进行RHS，并对bLHS。return a + b(对a和b进行RHS)。var c = ...(对c进行LHS)
